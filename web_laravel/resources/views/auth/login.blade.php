...
<head>
    ...
    <title>Login - {{ $setting->site_name ?? 'MenuKu' }}</title>
    @if($setting->logo)
        ...
        <div class="bg-white rounded-[2.5rem] p-10 shadow-xl shadow-slate-200/60 border border-slate-100">
            ...
            <form action="{{ route('login.post') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative group">
                        ...
                        <button type="button" onclick="togglePassword()" class="absolute right-4 top-1/2 -translate-y-1/2 text-slate-400 hover:text-slate-600 transition">
            ...
            <div class="mt-10 text-center">
                <p class="text-slate-500 text-sm font-medium">
                    Belum punya akun? 
                    <a href="/register" class="text-brand font-black hover:underline">Daftar Sekarang</a>
                </p>
            </div>
        </div>

        <p class="mt-10 text-center text-slate-400 text-xs font-medium uppercase tracking-[0.2em]">
            &copy; 2026 MenuKu SaaS &bull; Digitalizing Gastronomy
        </p>
    </div>
